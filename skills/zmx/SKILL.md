...
Use `wait` for finite tasks you detached manually; it returns their result and includes recent output for failures. Sessions named `pi-bg-*` belong to a Pi harness, which watches them itself: do not `zmx wait` on them, kill them, or manage their lifecycle; use `zmx history` only for read-only inspection when the user asks. For a long-running service, inspect startup through `history` or `tail` instead of waiting for it to exit.
