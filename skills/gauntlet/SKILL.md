...
Run the current diff through a gauntlet of four independent review passes at once — one Claude `/code-review` and three Codex `review` passes (`broad`, `failure-modes`, `contracts`) — merge their findings, **fix the major issues by default**, and summarize. Findings raised by **more than one pass** are the highest-confidence ones.

The parallel launch is handled by the bundled `scripts/gauntlet-review` launcher; this skill owns the judgment around it.

This skill is tuned for **recall at generation time and precision at judgment time**. The passes are deliberately asked to be exhaustive, so expect more raw findings than pass the bar — including some false positives. That is the intended shape: a finding you never generate cannot be caught in step 4, but a bad one can. Do not try to restore precision by softening the launcher's prompts; restore it by verifying and declining in step 4.
...
Derive it from the conversation, the task, commit messages, or the PR description. If the intent is genuinely unclear, ask the user one question rather than guessing — a wrong brief produces wrong findings. The script passes this brief verbatim to every pass, and adds the scope and self-verification instructions itself.
...
- It runs all four passes in parallel and prints one delimited block per pass, labeled with that pass's state — `===== CLAUDE REVIEW [ok] =====` and `===== CODEX REVIEW [pass: broad] [ok] =====` (then `failure-modes`, `contracts`). **Reviews take 5–20 minutes. If the harness offers managed background execution that notifies on completion, launch the command that way and continue other work until the notification arrives. Only without such a mechanism, run it in the foreground with the longest timeout available.**
- Reasoning effort defaults to `high`; raise it for a single run with `GAUNTLET_CODEX_EFFORT` (for example `xhigh`) or change the model with `GAUNTLET_CODEX_MODEL`.
- Every run saves each pass's raw output, the reviewed SHA, and the brief under a per-repo/per-branch state dir, and prints a `===== GAUNTLET STATE =====` footer with the round number, the output path, and the ledger path. **Note those paths** — the raw output is how you re-check a dedupe decision without paying for another 5–20 minute round.
...
- **Graceful degradation:** a pass that is unauthenticated or out of usage/quota gets an `[unavailable: …]` block, and the script prints a `NOTE:` line saying what dropped, succeeding on whatever ran. Proceed with the available findings and tell the user what was skipped and why. Fewer passes means a weaker agreement signal, so lean harder on per-finding verification (step 4.1). If nothing ran the script prints `ERROR:` and exits non-zero — report that and stop.
...
- For each unique issue, record **how many passes raised it and which** (e.g. `claude + broad`, `contracts only`, `3/4`). The Codex passes are independent samples of the same reviewer, so agreement among them is a weaker signal than Claude and Codex agreeing — weight cross-engine agreement highest.
- **Agreement is a confidence signal** — surface multi-pass findings first, and treat a single-pass finding as a candidate that step 4.1 must earn.
- Codex scores P1/P2/P3, Claude scores high/medium/low — normalize to one scale (high/medium/low); on conflict take the higher.
- Expect the lensed passes to produce findings the broad pass missed entirely — especially `contracts` findings in files outside the diff. Those are the point of running them; do not discard a finding merely because only one pass saw it.

## 4. Fix the major issues

For each finding that is **major** (high severity / Codex P1–P2):

1. **Verify it against the real code first** — open the file and adjacent code and confirm the bug is real, in scope, and not already handled. Reading is usually enough; the gates were green before this run, so re-running them settles nothing. Probe directly only when the finding hinges on a case no test covers. This verification is the *primary* false-positive guard — the passes are asked to be exhaustive precisely because this step exists, so it is not optional and a healthy run declines a real share of findings.
...
|----------|----------|-------|------------|--------|
| high | `path:line` | one-line description | claude + broad | fixed |
| high | `path:line` | … | contracts | declined (false positive: …) |
| medium | `path:line` | … | claude | reported |

Close with a one-line tally: total unique findings, how many were cross-engine agreements, how many were fixed vs declined vs left as minor, and whether any pass failed to run.
...
That only works with a memory. Without one, round 2 spends four passes re-deriving round 1's findings and you dedupe the duplicates away — you paid for the round and got the remainder. The ledger is what turns the next round outward onto new ground, and it is the same mechanism the hosted reviewer uses.
...
Each round costs four passes at 5–20 minutes plus real Codex quota, so say so before starting a long loop on a large diff. Two or three rounds is the useful range for most changes.
