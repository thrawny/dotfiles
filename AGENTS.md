...
The additive Apple Silicon setup lives in `portable/macos/` and is applied by `bin/setup-macos`. Keep its plain configs aligned manually with the Nix modules. Test link logic with `just test-macos-portable`; use `just test-macos-container` when package installation or consumer startup can change.
