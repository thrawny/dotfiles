...
- `background-bash.ts` — adds `background: true` to `bash` using zmx, automatically backgrounds configured apps such as `live-html`, provides completion and non-destructive timeout wake-ups, and retains sessions plus `~/.cache/pi/background-bash/<session>.sh` control scripts for 12 hours
- `gc.ts` — provides `/gc` with inline Git context and a temporary `thinking: off` override
- `handoff.ts` — asks the live compacted agent for a focused transfer note, then opens a parent-linked review session with a bounded `agent-history query` fallback
